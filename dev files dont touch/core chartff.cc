...
<head>
    ...
    <style>
        ...
        *::after {
        ...
        .task-list.is-dragging .task-item * {
            pointer-events: none;
        }
...
<body>
    ...
    <div class="top-navbar">
        <input type="file" id="import-file-input" style="display: none;" accept=".db,.json" onchange="importDBFile(event)">
        <button class="nav-btn" onclick="location.reload()">🏠 Home</button>
        <button class="nav-btn" onclick="triggerImport()">📥 Import DB</button>
        <button class="nav-btn" onclick="exportDBFile()">📤 Export DB</button>
        <button class="nav-btn" onclick="exportChartAsPNG()">📷 Export PNG</button>
        <button class="nav-btn" style="margin-left: auto;" onclick="openAboutModal()">ℹ️ About</button>
    ...
    <div class="simulator-card">
        <div class="header">
            <div class="title-area" style="display: flex; flex-direction: column; gap: 10px; align-items: flex-start;">
                <h1 id="project-title" contenteditable="true" spellcheck="false" class="editable-box">Gantt Chart Maker</h1>
                <p id="project-subtitle" contenteditable="true" spellcheck="false" class="editable-box"
                    style="margin: 0; color: var(--text-dim); font-size: 14px;">Create dynamic project timelines
                    instantly.</p>
            </div>
            <div class="stats-area">
                <div class="stat-box">
                    <div id="label-total-tasks" class="stat-label editable-box" contenteditable="true" spellcheck="false"
                ...
                <div class="stat-box">
                    <div id="label-total-weeks" class="stat-label editable-box" contenteditable="true" spellcheck="false"
                    ...
                    <div class="stat-value">
                        ...
                        <span id="label-wks" class="editable-box" contenteditable="true" spellcheck="false"
        ...
        <div class="controls-panel">
            <div class="form-group">
                <label id="label-task-name" class="editable-box" contenteditable="true" spellcheck="false">Task Name</label>
                <input type="text" id="input-name" placeholder="E.g. Strategy Formulation">
            </div>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                <div class="form-group">
                    <label id="label-start-week" class="editable-box" contenteditable="true" spellcheck="false">Start (Week)</label>
                    <input type="number" id="input-start" value="1" min="1">
                </div>
                <div class="form-group">
                    <label id="label-duration" class="editable-box" contenteditable="true" spellcheck="false">Duration (Wks)</label>
                    <input type="number" id="input-dur" value="2" min="1">
                </div>
            </div>
            <div class="form-group">
                <label id="label-color" class="editable-box" contenteditable="true" spellcheck="false">Color</label>
            ...
            <div id="label-task-list" class="editable-box" contenteditable="true" spellcheck="false"
        ...
        <div class="chart-panel">
            <div style="display: flex; justify-content: flex-start; align-items: center; margin-bottom: 8px; padding: 0 40px;">
                <h2 id="chart-section-title" contenteditable="true" spellcheck="false" class="editable-box" style="margin: 0; font-size: 18px; font-weight: 700; color: var(--text-main);">Project Timeline</h2>
            </div>
    ...
    <div id="notes-modal" style="display: none; position: fixed; z-index: 1000; background: var(--card-bg); padding: 16px; border-radius: 12px; width: 320px; border: 1px solid var(--glass-border); box-shadow: 0 10px 40px var(--shadow-color); backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px);">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; gap: 8px;">
            <input type="text" id="notes-modal-title" style="flex: 1; margin: 0; font-size: 14px; font-weight: 700; color: var(--text-main); background: var(--input-bg); border: 1px solid var(--btn-ghost-border); border-radius: 6px; padding: 6px 10px; outline: none; transition: all 0.2s; box-sizing: border-box;" onfocus="this.style.borderColor='var(--primary-blue)'; this.style.background='var(--input-bg-focus)'" onblur="this.style.borderColor='var(--input-border)'; this.style.background='var(--input-bg)'">
            <button onclick="closeNotesModal()" style="background: none; border: none; color: var(--text-dim); font-size: 18px; cursor: pointer; padding: 0; line-height: 1; margin-left: 4px;" onmouseover="this.style.color='var(--text-main)'" onmouseout="this.style.color='var(--text-dim)'">&times;</button>
        </div>
        <textarea id="notes-modal-textarea" placeholder="Add notes here..." style="width: 100%; height: 120px; background: var(--input-bg); border: 1px solid var(--input-border); color: var(--text-main); padding: 10px; border-radius: 8px; font-family: inherit; font-size: 13px; resize: none; margin-bottom: 12px; box-sizing: border-box; outline: none; transition: border-color 0.2s;" onfocus="this.style.borderColor='var(--primary-blue)'; this.style.background='var(--input-bg-focus)'" onblur="this.style.borderColor='var(--input-border)'; this.style.background='var(--input-bg)'"></textarea>
    ...
    <script>
            ...
            tasksData.forEach(task => {
                ...
                el.ondragstart = (e) => {
                    ...
                    setTimeout(() => {
                        ...
                        taskListContainer.classList.add('is-dragging');
                    }, 0);
                };
                ...
                el.ondragend = () => {
                    ...
                    taskListContainer.classList.remove('is-dragging');
                ...
                el.innerHTML = `
                <div class="grab-handle" onmousedown="this.parentElement.setAttribute('draggable', true)" onmouseup="this.parentElement.removeAttribute('draggable')" style="cursor: grab; color: var(--text-dim); padding-right: 10px; font-weight: bold; font-size: 16px; user-select: none; display: flex; align-items: center;" title="Drag to reorder">&#x28FF;</div>
                <div class="task-item-info" style="flex: 1; margin-right: 12px; min-width: 0;">
                    ...
                    <div style="display: flex; gap: 8px; font-size: 11px; color: var(--text-dim); align-items: center; flex-wrap: wrap;">
                        <span style="white-space: nowrap; display: flex; align-items: center; gap: 4px;" class="editable-box" contenteditable="true" spellcheck="false">
                            Wk <input type="number" class="inline-input" value="${task.start + 1}" min="1" onchange="updateTask(${task.id}, 'start', this.value - 1)" style="width: 45px; color: var(--text-main); display: inline-block;">
                        </span>
                        <span style="white-space: nowrap;" class="editable-box" contenteditable="true" spellcheck="false">
                            - Wk ${task.start + task.dur + 1}
                        </span>
                        <span style="white-space: nowrap; display: flex; align-items: center; gap: 4px;" class="editable-box" contenteditable="true" spellcheck="false">
                    ...
                    <div style="margin-top: 8px; font-size: 11px; color: var(--text-dim); background: var(--glass-bg); padding: 6px 8px; border-radius: 6px; border: 1px solid var(--glass-border); line-height: 1.4; word-break: break-word; cursor: pointer;" title="Click colored bar on the timeline to edit this note" onclick="openNotesModal(${task.id}, event)">
                        <span style="opacity: 0.7;">📝</span> <i style="margin-left: 2px;">${task.notes.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</i>
                    </div>` : ''}
                </div>
        ...
        function addTask() {
            ...
            const start = Math.max(0, (parseInt(startNode.value) || 1) - 1);
            ...
            nameNode.value = '';
            startNode.value = (start + dur + 1).toString();
            durNode.value = '2';
        ...
        function updateChart() {
            ...
            // Adjust chart width dynamically, enforcing a physical pixel minimum to prevent text crushing on small window sizes
            const minGridWidth = chartWeeks * 45; // Minimum 45 physical pixels per week
            chartInner.style.minWidth = `max(100%, ${minGridWidth}px)`;
            ...
            for (let i = 0; i <= chartWeeks; i++) {
                ...
                line.innerHTML = `<span class="editable-box" style="position: absolute; top: 35px; transform: translateX(-50%); font-size: 12px; font-weight: 600; color: var(--text-dim); text-align: center; background: var(--bg-dark); white-space: nowrap; z-index: 2;" contenteditable="true" spellcheck="false">Wk ${i + 1}</span>`;
        ...
        function exportChartAsPNG() {
            ...
            const originalBorderTop = chartNode.style.borderTop;
            
            // To ensure the PNG has a solid transparent mapping
            const themeBg = document.body.getAttribute('data-theme') === 'light' ? '#E4E1D9' : '#0f172a'; chartNode.style.background = themeBg;
            
            // Temporarily insert title so it gets cleanly captured in export PNG
            chartNode.style.borderTop = '70px solid transparent';
            const originalTitleBtn = document.getElementById('chart-section-title');
            const clonedTitle = document.createElement('h2');
            clonedTitle.innerHTML = originalTitleBtn.innerHTML;
            clonedTitle.id = 'temp-export-title';
            clonedTitle.style.cssText = "position: absolute; top: -55px; left: 0px; font-size: 24px; font-weight: 700; color: var(--text-main); margin: 0; z-index: 100;";
            chartNode.appendChild(clonedTitle);
            ...
            html2canvas(chartNode, {
                ...
                chartNode.style.borderTop = originalBorderTop;
                const tmpNode = document.getElementById('temp-export-title');
